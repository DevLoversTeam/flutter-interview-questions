...
Flutter - це open-source UI toolkit від Google для створення
кросплатформених застосунків з єдиної кодової бази.
...
4. **Висока швидкість розробки:** Hot Reload дозволяє швидко бачити зміни в
   коді без повного перезапуску застосунку.
...
5. **Сильна екосистема:** Є велика кількість пакетів для навігації, стану,
   HTTP, локального зберігання, аналітики та тестування.
...
non-nullable типи і виявляти частину null-related помилок ще під час
компіляції.
...
Extension methods у Dart дозволяють додавати нові методи або геттери до
існуючих типів без зміни їхнього вихідного коду.
...
Якщо layout "ламається", майже завжди треба дивитися на constraints, які
отримує віджет від свого parent.
...
State у Flutter - це дані, від яких залежить те, як виглядає або поводиться UI
...
Flutter є декларативним фреймворком. Це означає, що UI описується як функція
від поточного state.

#### Практичний сенс:

Робота з Flutter значною мірою зводиться до правильної відповіді на питання:
"де має жити цей state, хто ним володіє і хто повинен на нього реагувати?"
...
Навігація у Flutter побудована навколо стеку маршрутів (routes). Коли
користувач переходить на новий екран, новий route додається у стек, а при
поверненні зі стека прибирається верхній route.
...
Це базові методи для навігації у більшості Flutter-застосунків і один із
перших API, який потрібно впевнено знати на співбесіді.
...
- Основні віджети: `CupertinoApp`, `CupertinoPageScaffold`,
...
У Dart найчастіше виділяють два основні типи streams: **single-subscription**
та **broadcast streams**.
...
Якщо джерело даних має одного споживача, зазвичай достатньо
single-subscription stream. Якщо потрібно розсилати події кільком слухачам,
використовують broadcast stream.
...
Саме через цю архітектуру Flutter дає високу консистентність UI між
платформами, але інколи вимагає окремої інтеграції з нативним кодом.
...
Це важливий інструмент для життєвого циклу, особливо у застосунках із
медіа, аналітикою, socket-з'єднаннями або background-sensitive логікою.
...
Якщо певний код не використовується, компілятор або білд-система не включає
його в релізну збірку.
...
Продуктивність у Flutter треба не "вгадувати", а вимірювати. Хороша
оптимізація майже завжди починається з профілювання, а не з випадкових змін.
...
Здорова тестова стратегія зазвичай комбінує всі три типи, а не покладається
лише на один.
...
Widget testing - це тестування віджетів Flutter у контрольованому середовищі
без запуску повного застосунку на реальному пристрої.
...
Add-to-app підхід корисний тоді, коли бізнес не готовий переписати все, але
хоче поступово використати Flutter у продукті.
...
Платформо-залежний код у Flutter реалізують кількома способами залежно від
рівня задачі.
...
Якщо є готовий перевірений plugin, краще використовувати його. Якщо немає,
тоді вже писати platform channel або власний plugin.
